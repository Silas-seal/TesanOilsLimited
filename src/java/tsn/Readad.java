 ...
 * @author SeumX Plus
 */
@WebServlet(name = "Readad", urlPatterns = {"/Readad"})
public class Readad extends HttpServlet {
                ...
                if(rs.next() && rs.getString("title").equals("administrator")) {
                    out.println(head);
                        ...
                        out.println("<td valign=\"top\"><table border=\"2\" cellpadding=\"4\" cellspacing=\"4\" bgcolor=\"darkred\">"
                                ...
                                //calling msg body
                    String qr="select *from mail where ID=?";
                    PreparedStatement p; p=con.prepareStatement(qr); p.setString(1,request.getParameter("id")); System.out.println(request.getParameter("id"));
                    ResultSet r=p.executeQuery(); 
                    
                    out.println("<table border=\"0\" cellpadding=\"5\" cellspacing=\"5\" bgcolor=\"white\">"
                            + "<tr><th colspan=\"3\"> </th> </tr>"
                            + "<tr><td bgcolor=\"purple\" valign=\"top\"><ul type=\"disc\" valign=\"top\">"
                            ...
                            + "</td><td valign=\"top\"> <table border=\"1\" cellpadding=\"3\" cellspacing=\"3\" bgcolor=\"aliceblue\">");
                    
                    while(r.next()) {
                        out.println("<td valign=\"top\"> "
                                + "<h2><font color=\"darkolivegreen\">Mail Body</font></h2>"
                                + "<h3>"+r.getString("m_tofro")+"</h3>"
                                + "<h4>"+r.getString("m_date")+"</h4>"
                                + "<h4>"+r.getString("m_subject")+"</h4>"
                                + "<font face=\"Cambria\" size=\"+1\" color=\"darkred\">"+r.getString("m_content")+"</font> <p/>"
                                + "<form action=\"Delmailad\" method=\"post\">"
                                + "<input type=\"hidden\" name=\"id\" value=\""+r.getString("ID")+"\" />"
                                + "<button type=\"submit\" style=\"background-color:navy\"><font face=\"Cambria\" size=\"+1\" color=\""
                                + "azure\">DELETE MAIL </button> </form>"
                                + "</td> </tr>");
                    }
                    
                    String upq="update mail set m_type=\"read\" where ID=?";
                    PreparedStatement ps; ps=con.prepareStatement(upq); ps.setString(1,request.getParameter("id"));
                    ps.executeUpdate();
