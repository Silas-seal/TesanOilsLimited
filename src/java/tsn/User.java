        ...
        try (PrintWriter out = response.getWriter()) {
            try {
                ...
                if(rs.next() && rs.getString("title").equals("administrator")) {
                    ...
                    PreparedStatement pp=con.prepareStatement("select *from user"); ResultSet rr=pp.executeQuery();
                    if(rr.next()) {
                    if(!request.getParameter("uname").equals(rr.getString("name"))) {
                        String qryu="insert into user(name,title,contact,branch,password,email) Values(?,?,?,?,?,?)";
                        PreparedStatement ps1=con.prepareStatement(qryu); ps1.setString(1,request.getParameter("uname"));
                        ps1.setString(2,request.getParameter("utit")); ps1.setString(3,request.getParameter("ucont"));
                        ps1.setString(4,request.getParameter("ubr")); ps1.setString(5,request.getParameter("upas")); 
                        ps1.setString(6,request.getParameter("umail"));
                        ps1.executeUpdate();
                    } }
                    }
                    ...
                    out.println(head);
                        ...
                        out.println("<td valign=\"top\"><table border=\"2\" cellpadding=\"4\" cellspacing=\"4\" bgcolor=\"darkred\">"
                                ...
                                + "</font></button></a></th>"
                                + "<th valign=\"top\" bgcolor=\"aliceblue\"><a href=\"User\"><button type=\"button\" style=\"background-color:darkred\"><font color=\"azure\" size=\"+2\" face=\"Cambria\">USERS"
                                + "</font></button></a></th>"
                        ...
                        out.println("<b>REGISTER NEW SYSTEM USER</b> <p/>"
                                + "<form action=\"User\" method=\"post\"><table border=\"1\" cellpadding=\"3\" cellspacing=\"3\" bgcolor=\"cyan\">"
                                + "<tr><td><input type=\"text\" name=\"uname\" placeholder=\"Enter Username\" /></td>"
                                + "<td><input type=\"text\" name=\"ucont\" placeholder=\"Enter Contact\" /></td>"
                                + "<td><select type=\"dropdown\" name=\"utit\">"
                                + "<option value=\"administrator\" selected>Administrator</option>"
                                + "<option value=\"manager\" selected>Manager</option> </select></td></tr>"
                                + "<tr><td><input type=\"text\" name=\"umail\" placeholder=\"Email e.g John@tesan.org\" /></td>"
                                + "<td><input type=\"text\" name=\"ubr\" placeholder=\"Enter Branch\" /></td>"
                                + "<td><input type=\"text\" name=\"upas\" placeholder=\"Enter Password\" /></td>"
                                + "<td><button type=\"submit\" style=\"background-color:navy\"><font face=\"Cambria\" size=\"+1\" color=\"azure\">REGISTER"
                                + "</font> </button>"
                                
                                + "</table></form> <p/>");
                        
                        //deleteing system user
                        if(request.getParameter("delete")!=null) {
                            String dl="delete from user where ID=?"; 
                    ...
                    out.println("<table border=\"1\" cellpadding=\"3\" cellspacing=\"3\" bgcolor=\"plum\" width=\"978\">"
                            + "<tr><th>ID</th><th>NAME</th><th>TITLE</th><th>CONTACT</th><th>EMAIL</th><th>PASSWORD</th><th>BRANCH</th>"
                            + "<th>DELETE</th></tr>");
                    
                    while(r.next()) {
                        out.println("<tr><td>"+r.getString("ID")+"</td><td>"+r.getString("name")+"</td><td>"+r.getString("title")+"</td>"
                                + "<td>"+r.getString("contact")+"</td><td>"+r.getString("email")+"</td><td>"+r.getString("password")+"</td>"
                                        + "<td>"+r.getString("branch")+"</td>"
                    + "<td><form action=\"User\" method=\"post\"> <input type=\"hidden\" name=\"delete\" value=\""+r.getString("ID")+"\" />"
